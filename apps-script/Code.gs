...
function bridgeResponse_(obj,requestId){const payload=JSON.stringify({requestId:String(requestId||''),result:obj}).replace(/</g,'\\u003c');return HtmlService.createHtmlOutput('<!doctype html><meta charset="utf-8"><script>top.postMessage('+payload+',"https://stepkobetsu-hub.github.io");<\\/script>').setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);}
