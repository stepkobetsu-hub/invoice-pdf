...
function enqueueSend_(payload, requestAuth) {
  ...
  try {
    ...
    numbers.forEach(number => {
      ...
      const settings=settings_(), expires=new Date(now.getTime()+Number(settings.validDays||180)*86400000);
...
function sendOne_(queueValues, queueMap, settings) {
  ...
  const values={'取引先名':d[m['宛名']],'敬称':invoice.敬称||'様','顧客コード':d[m['顧客コード']],'対象年月':invoice.対象年月||'','請求書番号':d[m['請求書番号']],'請求金額':Number(invoice.請求金額||0).toLocaleString('ja-JP')+'円','支払期限':invoice.支払期限||'','有効日数':settings.validDays||180,'有効期限':formatDate_(d[m['トークン有効期限']]),'ダウンロードURL':url,'事業者名':'個別指導ステップ','電話番号':'0568-41-8937','返信先メールアドレス':settings.replyTo||'stepkobetsu@gmail.com'};
