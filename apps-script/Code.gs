...
function getDeliveryDiagnostics_(invoiceNumber,requestAuth){
  ...
  requests.push(Object.assign({url:'https://api.brevo.com/v3/account'},requestOptions));requestKeys.push('account');
  ...
  if(responseByKey.account){const accountBody=responseByKey.account.body,sendPlan=(Array.isArray(accountBody.plan)?accountBody.plan:[]).find(plan=>String(plan.creditsType||'')==='sendLimit'&&String(plan.type||'')!=='sms');result.account={httpStatus:responseByKey.account.code,relayEnabled:accountBody.relay?.enabled===true,credits:sendPlan&&Number.isFinite(Number(sendPlan.credits))?Number(sendPlan.credits):null,planType:sendPlan?String(sendPlan.type||''):''};if(result.account.relayEnabled===false){result.providerError='BREVO_RELAY_DISABLED';return result;}if(result.account.credits===0){result.providerError='BREVO_CREDITS_EXHAUSTED';return result;}}
