...
const STEP = Object.freeze({
  ...
  INVOICE_HEADERS: ['請求書番号','顧客コード','宛名','敬称','対象年月','請求日','支払期限','郵便番号','住所','税抜小計','消費税額','請求金額','メールアドレス','CCメールアドレス','PDF状態','PDFファイルID','PDFファイル名','現在状態','作成日時','更新日時','メモ','タグ','入金状態'],
...
function doPost(e) {
  try {
    ...
    const routes = {
      ...
      updatePaymentStatus: () => updatePaymentStatus_(payload.invoiceNumber, payload.paymentStatus, requestAuth), deleteInvoice: () => deleteInvoice_(payload.invoiceNumber, requestAuth),
...
function updatePaymentStatus_(invoiceNumber, paymentStatus, requestAuth) {
  requirePermission_('PDF作成', requestAuth);
  ensureInvoiceColumns_();
  const number=String(invoiceNumber||''),status=String(paymentStatus||'');
  ...
  const before=String(row.values[t.map['入金状態']]||'未入金');
  updateRow_(sh,row.rowNumber,t.map,{'入金状態':status,'更新日時':new Date()});
  log_('入金状態変更',number,String(row.values[t.map['顧客コード']]||''),'','成功','',{status:before},{status:status});
  return {invoiceNumber:number,paymentStatus:status};
}
