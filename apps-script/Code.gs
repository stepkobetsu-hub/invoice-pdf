...
function getDeliveryDiagnostics_(invoiceNumber,requestAuth){
  ...
  try{
  const requestOptions={method:'get',headers:{'api-key':apiKey,accept:'application/json'},muteHttpExceptions:true},requests=[Object.assign({url:'https://api.brevo.com/v3/account'},requestOptions)],requestKeys=['account'];
  ...
  const accountBody=responseByKey.account?responseByKey.account.body:{},sendPlan=(Array.isArray(accountBody.plan)?accountBody.plan:[]).find(plan=>String(plan.creditsType||'')==='sendLimit'&&String(plan.type||'')!=='sms');
  result.account={httpStatus:responseByKey.account?responseByKey.account.code:0,relayEnabled:accountBody.relay?.enabled===true,credits:sendPlan&&Number.isFinite(Number(sendPlan.credits))?Number(sendPlan.credits):null,planType:sendPlan?String(sendPlan.type||''):''};
