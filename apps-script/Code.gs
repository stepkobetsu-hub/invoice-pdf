...
function getDeliveryDiagnostics_(invoiceNumber,requestAuth){
  ...
  if(senderDomain){const domainResponse=UrlFetchApp.fetch('https://api.brevo.com/v3/senders/domains/'+encodeURIComponent(senderDomain),{method:'get',headers:{'api-key':apiKey,accept:'application/json'},muteHttpExceptions:true});let domainBody={};try{domainBody=JSON.parse(domainResponse.getContentText()||'{}');}catch(_){}result.senderDomain={domain:senderDomain,httpStatus:domainResponse.getResponseCode(),verified:domainBody.verified===true,authenticated:domainBody.authenticated===true};}
  let body={};
  ...
  const logResponse=UrlFetchApp.fetch('https://api.brevo.com/v3/smtp/emails?limit=100&sort=desc&email='+encodeURIComponent(recipient),{method:'get',headers:{'api-key':apiKey,accept:'application/json'},muteHttpExceptions:true});
  let logBody={};try{logBody=JSON.parse(logResponse.getContentText()||'{}');}catch(_){logBody={};}
  ...
  if(nearbyLogs.length){const closest=nearbyLogs.reduce((best,item)=>Math.abs(new Date(item.date||item.createdAt).getTime()-sentMs)<Math.abs(new Date(best.date||best.createdAt).getTime()-sentMs)?item:best),uuid=String(closest.uuid||'');if(uuid){const detailResponse=UrlFetchApp.fetch('https://api.brevo.com/v3/smtp/emails/'+encodeURIComponent(uuid),{method:'get',headers:{'api-key':apiKey,accept:'application/json'},muteHttpExceptions:true});let detail={};try{detail=JSON.parse(detailResponse.getContentText()||'{}');}catch(_){}result.events=(Array.isArray(detail.events)?detail.events:[]).map(event=>({event:event.name,date:event.time,reason:event.reason||''})).reverse();result.providerMessageId=String(closest.messageId||result.providerMessageId);result.providerStatus=detailResponse.getResponseCode();result.matchedBy='recipientLog';if(result.events.length)return result;}}
  ...
  const recipientEvents=Array.isArray(recipientBody.events)?recipientBody.events:[],nearby=Number.isFinite(sentMs)?recipientEvents.filter(event=>{const eventMs=new Date(event.date||'').getTime();return Number.isFinite(eventMs)&&Math.abs(eventMs-sentMs)<=15*60*1000;}):[];
