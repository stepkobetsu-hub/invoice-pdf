...
(async()=>{
  ...
  assert.ok(document.querySelector('#invoiceBulkAction'));
  assert.equal(document.querySelector('#invoiceVisibleRange').textContent,'1～2件を表示');
  assert.equal(document.querySelectorAll('#invoiceList [data-invoice-check]').length,2);
  document.querySelector('#invoiceSelectAll').click();
  assert.equal(document.querySelector('#invoiceBulkAction').disabled,false);
  assert.match(document.querySelector('#invoiceBulkAction').options[0].textContent,/選択した2件/);
  document.querySelector('#invoiceBulkAction').value='mail';
  document.querySelector('#invoiceBulkAction').dispatchEvent(new window.Event('change',{bubbles:true}));
  assert.equal(document.querySelector('#bulkMailWarningDialog').open,true);
  assert.equal(document.querySelector('#bulkMailTotalCount').textContent,'2件');
  document.querySelector('#reviewBulkMail').click();
  ...
  window.StepInvoiceApp.state.invoices=Array.from({length:205},(_,index)=>({...originalInvoices[0],invoiceNumber:String(300000000+index),createdAt:`2026-08-${String(10-(index%8)).padStart(2,'0')}`}));
  window.StepInvoiceApp.state.invoiceVisibleLimit=100;
  window.StepInvoiceApp.renderInvoices();
  assert.equal(document.querySelectorAll('#invoiceList .invoice-list-item').length,100);
  assert.equal(document.querySelector('#invoiceVisibleRange').textContent,'1～100件を表示');
  document.querySelector('#loadMoreInvoices').click();
  assert.equal(document.querySelectorAll('#invoiceList .invoice-list-item').length,200);
  assert.equal(document.querySelector('#invoiceVisibleRange').textContent,'1～200件を表示');
  window.StepInvoiceApp.state.invoices=originalInvoices;
  window.StepInvoiceApp.state.invoiceVisibleLimit=100;
  window.StepInvoiceApp.renderInvoices();
  assert.match(document.querySelector('#invoiceList .invoice-list-item:first-child').textContent,/最新の取引先/);
  assert.match(document.querySelector('#invoiceList .invoice-list-item:first-child').textContent,/下書き/);
  assert.match(document.querySelectorAll('#invoiceList .invoice-list-item')[1].textContent,/送信済/);
  assert.doesNotMatch(document.querySelectorAll('#invoiceList .invoice-list-item')[1].textContent,/DL済/);
  window.StepInvoiceApp.state.invoiceDownloadedNumbers.add('202608102');
  window.StepInvoiceApp.renderInvoices();
  assert.match(document.querySelector('#invoiceList .invoice-list-item:first-child').textContent,/DL済/);
  assert.doesNotMatch(document.querySelector('#invoiceList .invoice-list-item:first-child').textContent,/下書き/);
  window.StepInvoiceApp.state.invoiceDownloadedNumbers.delete('202608102');
  window.StepInvoiceApp.renderInvoices();
  ...
  assert.match(document.querySelector('#invoiceDetailPanel').textContent,/PDF/);
  assert.doesNotMatch(document.querySelector('#invoiceDetailPanel').textContent,/PDF／印刷/);
  ...
  assert.equal(document.querySelector('#saveSingleInvoice').classList.contains('hidden'),false);
  assert.equal(document.querySelector('#saveSingleInvoice').textContent,'保存する');
