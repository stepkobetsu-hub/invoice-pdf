...
(async()=>{
  ...
  assert.equal(document.querySelector('#sidebarResizer').getAttribute('role'),'separator');
  assert.equal(document.querySelector('#invoiceListResizer').getAttribute('role'),'separator');
  assert.equal(document.querySelector('.app-shell').style.getPropertyValue('--sidebar-width'),'232px');
  assert.equal(document.querySelector('#invoiceWorkspace').style.getPropertyValue('--invoice-list-width'),'320px');
  document.querySelector('#sidebarResizer').dispatchEvent(new window.KeyboardEvent('keydown',{key:'ArrowLeft',bubbles:true}));
  document.querySelector('#invoiceListResizer').dispatchEvent(new window.KeyboardEvent('keydown',{key:'Home',bubbles:true}));
  assert.equal(document.querySelector('.app-shell').style.getPropertyValue('--sidebar-width'),'222px');
  assert.equal(document.querySelector('#invoiceWorkspace').style.getPropertyValue('--invoice-list-width'),'100px');
  assert.equal(document.querySelector('.app-shell').classList.contains('sidebar-compact'),false);
  assert.equal(document.querySelector('#invoiceWorkspace').classList.contains('invoice-list-compact'),true);
  document.querySelector('#sidebarResizer').dispatchEvent(new window.KeyboardEvent('keydown',{key:'Home',bubbles:true}));
  assert.equal(document.querySelector('.app-shell').style.getPropertyValue('--sidebar-width'),'64px');
  assert.equal(document.querySelector('.app-shell').classList.contains('sidebar-compact'),true);
  assert.equal(window.localStorage.getItem('stepInvoiceSidebarWidth'),'64');
  assert.equal(window.localStorage.getItem('stepInvoiceListWidth'),'100');
  ...
  assert.notEqual(document.querySelector('#singlePartner').value,'');
  document.querySelector('#createInvoiceFromList').click();
  await new Promise(resolve=>window.setTimeout(resolve,50));
  assert.equal(document.querySelector('#singlePartner').value,'');
  assert.equal(document.querySelector('#singlePartnerSearch').value,'');
  ...
  assert.match(styles,/\.invoice-detail-card\{overflow:visible;/);
  assert.match(styles,/\.invoice-detail-top \.payment-dropdown\{z-index:60\}/);
  assert.match(styles,/\.invoice-detail-top \.convert-dropdown\{z-index:50\}/);
  ...
  assert.equal(demoRows[1].querySelector('[name="detailName"]').value,'テスト');
  ...
  assert.match(document.querySelector('#singleInvoiceLivePreview').textContent,/テスト/);
