...
async function main(){
  ...
  assert.match(styles,/\.invoice-list-subject \.invoice-app-downloaded\{[^}]*font-size:9px/);
  assert.match(appSource,/function invoiceOpenBadge\(invoice\)/);
  assert.match(appSource,/level===1\?'':level/);
  assert.doesNotMatch(appSource,/URL確認/);
  assert.match(appSource,/invoice\.appDownloadedAt\?'<span class="status downloaded invoice-app-downloaded">DL済<\/span>'/);
  assert.match(appSource,/invoice-list-subject[^`]*\$\{downloaded\}<\/span><span class="invoice-list-statuses"/);
  assert.match(appSource,/\/downloaded`,\{method:'POST'\}/);
  assert.match(window.StepInvoiceApp.invoiceOpenBadge({emailOpenedAt:'2026-08-12T01:00:00Z'}),/>開封<\/span>/);
  assert.match(window.StepInvoiceApp.invoiceOpenBadge({emailOpenedAt:'x',urlOpenedAt:'y'}),/>開封2<\/span>/);
  assert.doesNotMatch(window.StepInvoiceApp.invoiceOpenBadge({emailOpenedAt:'x',urlOpenedAt:'y'}),/>開封<\/span>/);
  assert.match(window.StepInvoiceApp.invoiceOpenBadge({emailOpenedAt:'x',urlOpenedAt:'y',pdfViewedAt:'z'}),/>開封3<\/span>/);
  assert.doesNotMatch(window.StepInvoiceApp.invoiceOpenBadge({emailOpenedAt:'x',urlOpenedAt:'y',pdfViewedAt:'z'}),/>開封2<\/span>/);
