(function(global){
  ...
  function buildManualInvoice(values,detailRows,partner){
    ...
    return {
      ...
      customerCode:String(partner['顧客コード']||''),note:String(values.note||''),bank:String(values.bank||''),details:details.map(({taxRateValue,...row})=>row),pdfStatus:'未作成',sendStatus:'未送信',dlStatus:'未取得',
