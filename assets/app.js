(function(){
  ...
  const state={partners:[],invoices:[],history:[],preview:null,selected:new Set(),pendingResend:null,pendingDisable:null,resendSubmitting:false,settings:JSON.parse(localStorage.getItem('stepInvoiceSettings')||'{}')};
  ...
  async function refreshAll(){try{const data=await api('getDashboard');state.invoices=data.invoices||state.invoices;state.history=data.history||[];renderInvoices();renderHistory();$('#userLabel').textContent=data.user||'接続済み';}catch(e){alert(e.message,'error');}}
  ...
  restoreForms();renderPartners();renderCreate();renderInvoices();renderHistory();
})();
