(function(){
  ...
  function renderInvoices(){const tbody=$('#invoiceTable tbody');tbody.innerHTML=state.invoices.length?state.invoices.map((x,i)=>`<tr><td><input type="checkbox" data-send-select="${i}" ${state.selected.has(i)?'checked':''}></td><td>${esc(x.invoiceNumber)}</td><td>${esc(x.customerCode)}</td><td>${esc(x.partnerName)}</td><td>${esc(x.email)}</td><td>${esc(x.cc)}</td><td class="num">${C.formatYen(x.total)}</td><td>${esc(x.pdfStatus)}</td><td><span class="status ${statusClass(x.sendStatus)}">${esc(x.sendStatus)}</span></td><td>${esc(x.sentAt||'')}</td><td><span class="status ${statusClass(x.dlStatus)}">${esc(x.dlStatus)}</span></td><td>${esc(x.downloadedAt||'')}</td><td>${esc(x.expiresAt||'')}</td><td><button data-row-action="preview" data-i="${i}">プレビュー</button> <button data-row-action="send" data-i="${i}">送信</button> ${C.isSentStatus(x.sendStatus)?`<button data-row-action="resend" data-i="${i}">再送</button>`:''} <button data-row-action="disable" data-i="${i}">配信停止</button></td></tr>`).join(''):'<tr class="empty"><td colspan="14">請求書がありません。</td></tr>';tbody.querySelectorAll('[data-send-select]').forEach(x=>x.onchange=()=>x.checked?state.selected.add(Number(x.dataset.sendSelect)):state.selected.delete(Number(x.dataset.sendSelect)));tbody.querySelectorAll('[data-row-action]').forEach(b=>b.onclick=()=>rowAction(b.dataset.rowAction,Number(b.dataset.i)));}
