(function(global){
  ...
  function pageHtml(inv,options={}){
    ...
    const detailPages=[];
    for(let index=0;index<allDetails.length;index+=8)detailPages.push(allDetails.slice(index,index+8));
    ...
    return detailPages.map((detailItems,pageIndex)=>{
      ...
      return `<div class="invoice-page"${pageIndex===0?' id="invoicePage"':''} style="--detail-count:${Math.max(1,detailItems.length)}">
      ${watermark}
