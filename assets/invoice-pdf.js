(function(global){
  ...
  function pageHtml(inv){
    const details=(inv.details||[]).slice(0,8).map(d=>`<tr><td></td><td>${esc(d.name)}</td><td class="num">${Number(d.unitPrice).toLocaleString()}</td><td class="num">${Number(d.quantity).toLocaleString()}</td><td class="num">${Number(d.amount).toLocaleString()}</td></tr>`).join('');
    return `<div class="invoice-page" id="invoicePage">
      ...
      <img class="logo" src="assets/step-logo.jpg" alt="STEP"><div class="abs issuer">個別指導ステップ<br><br>〒487-0024<br>愛知県春日井市大留町1丁目23-2<br>TEL: 0568-41-8937<br>${esc(inv.customerCode)}</div>
  ...
  async function toBlob(element){
    ...
    const canvas=await global.html2canvas(element,{scale:2,backgroundColor:'#fff',useCORS:true,logging:false});
    const pdf=new global.jspdf.jsPDF({orientation:'portrait',unit:'pt',format:'a4',compress:true});
    pdf.addImage(canvas.toDataURL('image/jpeg',.94),'JPEG',0,0,595.28,841.89,undefined,'FAST');
