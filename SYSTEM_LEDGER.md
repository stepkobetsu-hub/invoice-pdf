...
## 2026-08-10 100件一括送信の高速・安全化（実装試験中）

- 個人GmailのMailApp上限を回避するため、Brevo Transactional Email APIの100件バッチ送信へ移行する。
- Cloudflare配信URLを並列発行し、Brevoへの送信要求は100件を1回にまとめる。
- バッチID、二重送信防止キー、最大3回再試行、件数別進捗を追加した。
- 10件超のテストは自動的にBrevo Sandboxとし、実メールを送らない。
- 従来MailAppはAPIキー未設定時のフォールバックとして維持する。
- 詳細、設定、試験、復旧方法は `docs/BULK_EMAIL_100.md` を正本とする。
- 本番有効化は、Brevo APIキー設定、Sandbox 100件実測、少数実メール試験の合格後に行う。
